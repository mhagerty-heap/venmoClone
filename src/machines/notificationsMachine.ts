...
export const notificationsMachine = dataMachine("notifications").withConfig({
  services: {
    fetchData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.get(`https://venmo-clone.vercel.app:${backendPort}/notifications`, {
    ...
    updateData: async (ctx, event: any) => {
      ...
      const resp = await httpClient.patch(
        `https://venmo-clone.vercel.app:${backendPort}/notifications/${payload.id}`,
        payload
      );
      return resp.data;
    },
